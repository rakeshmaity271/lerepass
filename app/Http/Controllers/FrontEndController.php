...
class FrontEndController extends Controller
{
    ...
    public function restorant($alias)
    {
        ...
        if ($restorant->active == 1) {
            ...
            if (!Session::has('table_id')) {
                return redirect('restaurant/'.$alias);
            }
            // if ($restorant->user_register == 1 && !Session::has('visit_id')) {
            //     return redirect('register/visit/' .$restorant->id);
            // }
    ...
    {
        $this->validate($request, ['table_id' => 'required']);
        session()->put('table_id', $request->table_id);

        $restorant = Restorant::find($request->res_id);
        if ($restorant->user_register == 1) {
            return redirect('register/visit/' .$restorant->id);
        }
        return redirect('restaurants/'.$request->alias);

    }
    public function show_table($alias)
    {
        $subDomain = $this->getSubDomain();
        if ($subDomain && $alias !== $subDomain) {
            return redirect()->route('restorant', $subDomain);
        ...
        $restorant = Restorant::whereRaw('REPLACE(subdomain, "-", "") = ?', [str_replace("-", "", $alias)])->first();
        return view('restorants.choose_table', compact('restorant', 'alias'));
    }


    public function renderRegistrationHtml() {
        return view('html.registration');
    }
}
