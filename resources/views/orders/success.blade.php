            ...
            <div class="card-body text-center">
                ...
                <div class="d-flex justify-content-center">
                    <div class="col-8">
                        ...
                        <a href="{{ $order->restorant->getLinkAttribute() }}"
                            class="btn btn-outline-primary btn-sm">{{ __('Go back to restaurant') }}</a>
                        @else
                        <a href="{{ route('vendor',$order->restorant->subdomain) }}"
                            class="btn btn-outline-primary btn-sm">{{ __('Go back to restaurant') }}</a>
                        @endif

                        <!-- WHATS APP -->
                        @if ($showWhatsApp)
                        @if($order->restorant->whatsapp_only == 0)
                        <a target="_blank" href="?order={{$_GET['order']}}&whatsapp=yes"
