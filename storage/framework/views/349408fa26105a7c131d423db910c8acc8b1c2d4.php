    ...
    <div id="app" class="h-full md:flex md:justify-center md:items-center">
        <div class="w-full max-w-lg">
            ...
            <div class="bg-white rounded-lg shadow-xl p-4 sm:py-6 sm:px-10 mb-5">
                <?php if($payment->isSucceeded()): ?>
                    <h1 class="text-xl mt-2 mb-4 text-gray-700">
                        <?php echo e(__('Payment Successful')); ?>

                    </h1>

                    <p class="mb-6">
                        <?php echo e(__('This payment was already successfully confirmed.')); ?>

                    </p>
                <?php elseif($payment->isCancelled()): ?>
                    <h1 class="text-xl mt-2 mb-4 text-gray-700">
                        <?php echo e(__('Payment Cancelled')); ?>

                    </h1>

                    <p class="mb-6"><?php echo e(__('This payment was cancelled.')); ?></p>
                <?php else: ?>
                    <div id="payment-elements" v-if="! paymentProcessed">
                        ...
                        <div v-show="requiresPaymentMethod">
                            <!-- Instructions -->
                            <h1 class="text-xl mt-2 mb-4 text-gray-700">
                                <?php echo e(__('Confirm your :amount payment', ['amount' => $payment->amount()])); ?>

                            </h1>

                            <p class="mb-6">
                                <?php echo e(__('Extra confirmation is needed to process your payment. Please confirm your payment by filling out your payment details below.')); ?>
                            ...
                            <input
                                id="cardholder-name"
                                type="text" placeholder="<?php echo e(__('Jane Doe')); ?>"
                                required
                            ...
                            >
                                <?php echo e(__('Pay :amount', ['amount' => $payment->amount()])); ?>
                ...
                <button @click="goBack" ref="goBackButton" data-redirect="<?php echo e($redirect ?? url('/')); ?>"
    ...
    <script>
        ...
        var app = new Vue({
            ...
            data: {
                ...
                requiresPaymentMethod: <?php echo json_encode($payment->requiresPaymentMethod(), 15, 512) ?>,
                requiresAction: <?php echo json_encode($payment->requiresAction(), 15, 512) ?>,
                successMessage: '',
                errorMessage: ''
            },

            <?php if(! $payment->isSucceeded() && ! $payment->isCancelled() && ! $payment->requiresAction()): ?>
                mounted: function () {
            ...
            methods: {
                addPaymentMethod: function () {
                    ...
                    stripe.confirmCardPayment(
                        '<?php echo e($payment->clientSecret()); ?>', {
                            payment_method: {
                        ...
                        if (result.error) {
                            if (result.error.code === '<?php echo e(Stripe\ErrorObject::CODE_PARAMETER_INVALID_EMPTY); ?>' &&
                                ...
                                self.errorMessage = '<?php echo e(__('Please provide your name.')); ?>';
                            } else {
                            ...
                            self.successMessage = '<?php echo e(__('The payment was successful.')); ?>';
                        }
                    });
                },
                ...
                confirmPaymentMethod: function () {
                    ...
                    stripe.confirmCardPayment(
                        '<?php echo e($payment->clientSecret()); ?>', {
                            payment_method: '<?php echo e($payment->payment_method); ?>'
                        }
                    ).then(function (result) {
                        self.paymentProcessing = false;
                        ...
                        if (result.error) {
                            ...
                            if (result.error.code === '<?php echo e(Stripe\ErrorObject::CODE_PAYMENT_INTENT_AUTHENTICATION_FAILURE); ?>') {
...
<?php /**PATH /home/admin/web/lerepass.com/public_html/vendor/laravel/cashier/resources/views/payment.blade.php ENDPATH**/ ?>