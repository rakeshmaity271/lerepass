...
function initTheTrackingMap(currentPosiotion,){
    var map = new google.maps.Map(document.getElementById('tracking_map'), {
        zoom: 13,
        center:  new google.maps.LatLng(<?php echo e($order->address?$order->address->lat:$order->restorant->lat); ?>, <?php echo e($order->address?$order->address->lng:$order->restorant->lng); ?>),
    });

  //Marker Start - Restorant position
  var marker = new google.maps.Marker({
    position: new google.maps.LatLng(<?php echo e($order->restorant->lat); ?>, <?php echo e($order->restorant->lng); ?>),
    map: map,
    icon: start,
    title: '<?php echo e($order->restorant->name); ?>'
  });

  //Marker end - Client address
  var marker = new google.maps.Marker({
    position: new google.maps.LatLng(<?php echo e($order->address?$order->address->lat:$order->restorant->lat); ?>, <?php echo e($order->address?$order->address->lng:$order->restorant->lng); ?>),
    map: map,
    icon: end,
    title: '<?php echo e($order->client->name); ?>'
  });
...
function getOrderLocation(){
  axios.get('/ordertracingapi/<?php echo e($order->id); ?>').then(function (response) {
...
<style type="text/css">
  ...
  </style><?php /**PATH /home/admin/web/lerepass.com/public_html/resources/views/orders/partials/map.blade.php ENDPATH**/ ?>